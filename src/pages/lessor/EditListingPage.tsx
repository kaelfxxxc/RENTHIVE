...
export default function EditListingPage() {
  ...
  const loadListing = async () => {
    ...
    if (error || !data) { toastError("Not found", "Listing could not be loaded."); navigate("/lessor/listings"); return; }
    const l = data as Record<string, unknown>;
    setPrimaryImage(String(l.primary_image_url || "") || null);
    setForm({
      title: String(l.title || ""),
      category_id: String(l.category_id || ""),
      description: String(l.description || ""),
      condition: String(l.condition || "good"),
      price_per_day: String(l.price_per_day || ""),
      price_per_week: String(l.price_per_week || ""),
      security_deposit: String(l.security_deposit || ""),
      incidental_fee: String(l.incidental_fee || ""),
      location: String(l.location || ""),
      city: String(l.city || ""),
      rental_rules: String(l.rental_rules || ""),
      cancellation_policy: String(l.cancellation_policy || ""),
      pickup_available: Boolean(l.pickup_available ?? true),
      delivery_available: Boolean(l.delivery_available ?? false),
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    }).eq("id", id!).eq("lessor_id", user!.id);

    if (error) toastError("Failed to save", error.message);
    else { success("Listing updated"); navigate("/lessor/listings"); }
    setSaving(false);
  };
  ...
  return (
    <LessorLayout>
      <div className="max-w-2xl mx-auto">
        ...
        <h1 className="text-2xl font-bold mb-6" style={{ fontFamily: "var(--font-display)" }}>Edit Listing</h1>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="bg-white border border-[var(--border)] rounded-xl p-5 space-y-4">
            <h2 className="font-semibold">Basic Information</h2>
            <Input label="Product Title *" value={form.title} onChange={e => set("title", e.target.value)} />
            <Select
              label="Category"
          ...
          <div className="bg-white border border-[var(--border)] rounded-xl p-5 space-y-4">
            ...
            <ImageUploader label="Primary Image" value={primaryImage} onChange={setPrimaryImage} bucket="listings" folder="images" />
          </div>
          ...
          <div className="bg-white border border-[var(--border)] rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Input label="Incidental Fee (₱)" type="number" min="0" value={form.incidental_fee} onChange={e => set("incidental_fee", e.target.value)} />
            </div>
          </div>
          ...
          <div className="bg-white border border-[var(--border)] rounded-xl p-5 space-y-4">
            ...
            {form.delivery_available && (
              <Input label="Delivery Fee (₱)" type="number" min="0" value={form.delivery_fee} onChange={e => set("delivery_fee", e.target.value)} />
            )}
          </div>
